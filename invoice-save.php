...
function saveInvoice(array $data) {
    if (!empty($data)) {
        ...
        if (!empty($data)) {
            ...
            if (empty($invoice_id)) {
                $query = "INSERT INTO invoice (`invoice_id`, `customer_id`, `date`, `from_date`, `expire_date`, `customer_name`, `number`, `address`, `total_amount`, `gross_amount`, `discount`,`tax`,
							`additional_tax`,`notes`)
							VALUES (NULL, '$customer_id', STR_TO_DATE('$invoiceDate', '%m/%d/%Y'), STR_TO_DATE('$from_date', '%m/%d/%Y'), STR_TO_DATE('$expire_date', '%m/%d/%Y'), '$customer_name', '$phone', '$address', '$total_amount', '$gross_amount','$discount', '$tax', '$additional_tax','$notes')";
            } else {
                $query = "DELETE FROM invoice_details WHERE invoice_id='$invoice_id'";
                $query = "UPDATE invoice SET customer_id='$customer_id',date=STR_TO_DATE('$invoiceDate', '%m/%d/%Y'),from_date=STR_TO_DATE('$from_date', '%m/%d/%Y'),expire_date=STR_TO_DATE('$expire_date', '%m/%d/%Y'),customer_name='$customer_name',
                                        number='$phone',address='$address',total_amount='$total_amount',
							 gross_amount='$gross_amount',discount='$discount',tax='$tax',
							 additional_tax='$additional_tax',notes='$notes' WHERE invoice_id='$invoice_id'";
            }
            ...
            if (isset($data['data']) && !empty($data['data'])) {
                saveInvoiceDetail($data['data'], $invoice_id);
            }
            return [
                'success' => true,
